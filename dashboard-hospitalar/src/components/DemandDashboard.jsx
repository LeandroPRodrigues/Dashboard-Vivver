...
import { 
  Activity, Calendar, MapPin, Users, List, Clock, AlertTriangle, Stethoscope, 
  FileText, BriefcaseMedical, X
} from 'lucide-react';
...
export const DemandDashboard = ({ demandData }) => {
  ...
  const stats = useMemo(() => {
      ...
      const byCid = {}; const byService = {}; const byReqUnit = {}; const byGender = {};
      const byExecUnit = {}; const patientMap = {}; const byAgeCategory = {
          '0 a 30 dias': 0, '31 a 90 dias': 0, '91 a 365 dias': 0, 'Mais de 1 ano': 0
      };
      ...
      filteredDemand.forEach(item => {
          ...
          const prio = item.priority || 'NÃO CLASSIFICADO';
          byPriority[prio] = (byPriority[prio] || 0) + 1;
          if (prio.toUpperCase().includes('URGENTE') || prio.toUpperCase().includes('PRIORIDADE')) priorityCount++;
          ...
          const cid = item.cid || 'N/I'; byCid[cid] = (byCid[cid] || 0) + 1;
          ...
          const gen = item.gender || 'N/I'; byGender[gen] = (byGender[gen] || 0) + 1;
          const exeU = item.execUnit || 'Não Informada'; byExecUnit[exeU] = (byExecUnit[exeU] || 0) + 1;
          
          if (item.ageCategory) byAgeCategory[item.ageCategory]++;

          // Multiplas solicitações
          if (item.patientId) {
      ...
      // Transformação para arrays de gráficos
      ...
      return { 
          ...
          priorityChart: sortChart(byPriority), evolutionChart, topProcedures: sortChart(byProcedure).slice(0, 10),
          specialtyChart: sortChart(bySpecialty).slice(0, 10), cidChart: sortChart(byCid).slice(0, 15),
          serviceChart: sortChart(byService), reqUnitChart: sortChart(byReqUnit).slice(0, 15),
          genderChart: sortChart(byGender), execUnitChart: sortChart(byExecUnit).slice(0, 10),
          multiplePatientsChart, agingChart, rawTable: filteredDemand.slice(0, 100) // limite de 100 para a tabela
      };
  }, [filteredDemand]);
    ...
    <div className="flex flex-col gap-8 animate-in fade-in duration-500">
      ...
      <section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
               ...
               <Card className="p-6">
                   ...
                   <div id="chart-prioridade" className="h-64 w-full bg-white p-2"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stats.priorityChart} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={2} fill="#8884d8"><Cell fill="#ef4444"/><Cell fill="#f59e0b"/><Cell fill="#3b82f6"/><Cell fill="#94a3b8"/></Pie><RechartsTooltip /><Legend layout="horizontal" verticalAlign="bottom" wrapperStyle={{ fontSize: '11px' }}/></PieChart></ResponsiveContainer></div>
               </Card>
          </div>
      </section>
      ...
      <section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
               <Card className="p-6">
                   ...
                   <div id="chart-top-procs" className="h-80 w-full bg-white p-2"><ResponsiveContainer width="100%" height="100%"><BarChart layout="vertical" data={stats.topProcedures} margin={{ top: 5, right: 30, left: 100, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" hide /><YAxis dataKey="name" type="category" width={180} tick={{fontSize: 10}} interval={0} /><RechartsTooltip cursor={{fill: '#f1f5f9'}} /><Bar dataKey="value" name="Solicitações" fill="#10b981" radius={[0, 4, 4, 0]} barSize={15} /></BarChart></ResponsiveContainer></div>
               </Card>
               <Card className="p-6">
                   ...
                   <div id="chart-top-specs" className="h-80 w-full bg-white p-2"><ResponsiveContainer width="100%" height="100%"><BarChart layout="vertical" data={stats.specialtyChart} margin={{ top: 5, right: 30, left: 100, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" hide /><YAxis dataKey="name" type="category" width={180} tick={{fontSize: 10}} interval={0} /><RechartsTooltip cursor={{fill: '#f1f5f9'}} /><Bar dataKey="value" name="Solicitações" fill="#8b5cf6" radius={[0, 4, 4, 0]} barSize={15} /></BarChart></ResponsiveContainer></div>
               </Card>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <Card className="p-6 lg:col-span-1">
               ...
               <Card className="p-0 border-t-4 border-t-green-500 lg:col-span-2 overflow-hidden">
                   <div className="p-6 pb-4 bg-white flex justify-between items-center"><div><h3 className="text-lg font-bold text-slate-800 flex items-center gap-2"><FileText size={20} className="text-slate-400" /> CIDs Mais Frequentes (Justificativas)</h3></div><ExportWidget targetId="table-cids" fileName="tabela_cids" dataForExcel={stats.cidChart} /></div>
                   <div className="overflow-x-auto"><div id="table-cids" className="max-h-64 overflow-y-auto bg-white px-4 pb-4"><table className="w-full text-sm text-left text-slate-600"><thead className="text-xs text-slate-700 uppercase bg-slate-50 sticky top-0"><tr><th className="px-4 py-3 font-bold border-b">CID</th><th className="px-4 py-3 font-bold border-b text-right">Encaminhamentos</th></tr></thead><tbody className="divide-y divide-slate-100">{stats.cidChart.map((cid, i) => (<tr key={i} className="hover:bg-slate-50"><td className="px-4 py-2 font-mono text-xs">{cid.name}</td><td className="px-4 py-2 text-right font-bold text-slate-700">{cid.value.toLocaleString()}</td></tr>))}</tbody></table></div></div>
               </Card>
          </div>
      </section>
      ...
      <section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
               ...
               <Card className="p-6">
                   <div className="flex justify-between items-start mb-6"><h3 className="text-lg font-bold text-slate-800 flex items-center gap-2"><MapPin size={20} className="text-slate-400" /> Destino da Execução (Unid. Executora)</h3><ExportWidget targetId="chart-exec-unit" fileName="destino_execucao" /></div>
                   <div id="chart-exec-unit" className="h-64 w-full bg-white p-2"><ResponsiveContainer width="100%" height="100%"><BarChart layout="vertical" data={stats.execUnitChart} margin={{ top: 5, right: 30, left: 100, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" hide /><YAxis dataKey="name" type="category" width={180} tick={{fontSize: 9}} interval={0} /><RechartsTooltip cursor={{fill: '#f1f5f9'}} /><Bar dataKey="value" name="Encaminhamentos" fill="#0ea5e9" radius={[0, 4, 4, 0]} barSize={15} /></BarChart></ResponsiveContainer></div>
               </Card>
          </div>
                               ...
                               {stats.rawTable.map((req, i) => (
                                   <tr key={i} className="hover:bg-slate-50">
                                       ...
                                       <td className="px-4 py-2">
                                           <span className={`px-2 py-1 rounded text-[10px] font-bold ${String(req.priority).includes('URGENTE') ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700'}`}>
                                               {req.priority}
                                           </span>
                                       </td>
